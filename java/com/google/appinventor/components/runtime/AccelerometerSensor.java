...
public class AccelerometerSensor extends AndroidNonvisibleComponent implements OnStopListener, OnResumeListener, SensorComponent, SensorEventListener, Deleteable {
    ...
    private boolean isShaking(Queue<Float> cache, float currentValue) {
        ...
        float average2 = average / cache.size();
        if (Sensitivity() == 1) {
            return ((double) Math.abs(average2 - currentValue)) > strongShakeThreshold;
        }
        return Sensitivity() == 2 ? ((double) Math.abs(average2 - currentValue)) > moderateShakeThreshold && ((double) Math.abs(average2 - currentValue)) < strongShakeThreshold : ((double) Math.abs(average2 - currentValue)) > weakShakeThreshold && ((double) Math.abs(average2 - currentValue)) < moderateShakeThreshold;
    }
