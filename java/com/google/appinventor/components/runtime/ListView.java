...
@DesignerComponent(category = ComponentCategory.USERINTERFACE, description = "<p>This is a visible component that displays a list of text elements. <br> The list can be set using the ElementsFromString property or using the Elements block in the blocks editor. <br> Warning: This component will not work correctly on Screens that are scrollable.</p>", iconName = "images/listView.png", nonVisible = false, version = 3)
@SimpleObject
...
public final class ListView extends AndroidViewComponent implements AdapterView.OnItemClickListener {
    ...
    public ListView(ComponentContainer container) {
        ...
        this.listViewLayout = new android.widget.LinearLayout(container.$context());
    ...
    public Spannable[] itemsToColoredText() {
        ...
        for (int i = 1; i <= size; i++) {
            String itemString = this.items.get(i).toString();
